...
*   **End-to-End Implementation:** Building features, fixing bugs, or applying multi-file refactors with tests and commits across `base/`, `backend/`, `extension/`, or `app/`.
*   **Codebase Exploration:** Navigating the monorepo, tracing how the C API / WebSocket protocol / graph schema flows into each frontend, and summarizing architecture before you act.
*   **Agentic Tool Work:** Tasks that benefit from iterative shell/build/test loops (`pixi run test`, `uv run pytest`, `npm test`, `cargo test`) rather than pure text generation.
...
3.  **Strict Boundaries:** Specify format and limits (e.g., "Do not open a PR", "Output ONLY a bullet list of findings", "Modify only files under extension/src/").

**Example Usage:**
`grok -p 'Act as an expert TypeScript engineer. In the current repository, locate the webview graph panel in extension/ and identify why node selection is not highlighting the corresponding source range in the editor (a broken bidirectional-navigation link). Constraints: 1. Do not modify files. 2. Return a short root-cause analysis and a concrete patch suggestion in a single markdown code block.'`
